...
def resetUserWantHelp(chat_id, manager):
    sql.Customer.update({sql.Customer.link: manager}).where(sql.Customer.chat_id== chat_id).execute()

@app.route('/api/support/resetUserWantHelp/<chat_id>', methods=["GET"])
def resetUserWantHelp(chat_id):
    sql.Customer.insert(chat_id=chat_id, first_name="~", want_help='no', link="~").on_conflict('replace').execute()
    return {
        "chat_id": chat_id
    }
