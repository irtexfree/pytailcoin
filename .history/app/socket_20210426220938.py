...
def handle_stream_pending_support(json):    
    ...
    sql.Customer.update({sql.Customer.link: json.user}).where(sql.Customer.chat_id == user[0].chat_id).execute()
    bot.send_message(user[0].chat_id, 'Оператор на связи! Вы можете начать диалог с ним прямо сейчас')
