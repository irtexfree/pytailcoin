...
function x_function_close_current_chat() {
  $.getJSON(`/api/support/unlink/${window.x_data_chat.chat_id}`, function () {
    $.getJSON(`/api/support/unticket/${window.x_data_chat.chat_id}`, function () {
      window.x_data_chat.chat_id = 0; 
      window.dispatchEvent(new CustomEvent("chat-update", { detail: {
        chat_id: 0,
        chat: []
      } }));
    });  
...
$(function () {
  ...
  setInterval(() => {
    $.getJSON("/api/support/getUserWantHelp", function (data) {
      if (window.x_data_chat.chat_id == 0 || window.x_data_chat.chat_id == '0') 
