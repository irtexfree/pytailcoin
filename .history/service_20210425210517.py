from lbcapi import api
from app import sql

import requests

x = requests.get('https://localbitcoins.com/buy-bitcoins-online/.json')

# conn = api.hmac("", "")
# print(conn.call('GET', '/buy-bitcoins-online/.json').json())

for adventure in x.json()['data']['ad_list']:
    print(adventure['data'])
	sql.Adventure(id=adventure['data']['ad_id'], price=adventure['data']['temp_price'], amount=adventure['data']['max_amount_available'], provider=adventure['data']['max_amount_available']).save()
